...
public class GreenKartMultiBuy {
...
//	}
	...
	public void user_adds_multiple_veg_to_cart() {
		...
		Map<String, Integer> buyingList = new HashMap();
		buyingList.put("Beans", 2);
		buyingList.put("Carrot", 2);
		buyingList.put("Brinjal", 4);
		...
		for(int i = 0; i < pnames.size(); i++) {
			...
			if(buyingList.containsKey(formattedName)) {
				
				qty = buyingList.get(formattedName);
	...
	public void order_should_be_successfully_placed() {
		...
		Boolean titleDisplay = new WebDriverWait(driver, 10).until(ExpectedConditions.titleContains("GreenKart - veg and fruits kart"));
		
		Assert.assertTrue(titleDisplay);
